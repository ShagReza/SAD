...
#    (_, h, w, c) = x1.shape
    ...
    if atn:
        att_in = Bidirectional(GRU(config.P1_rnns[-1], return_sequences=True), merge_mode="concat", name='embed_1')(x1) 
    else:
        att_in = Bidirectional(GRU(config.P1_rnns[-1], return_sequences=False), merge_mode="concat", name='embed_1')(x1) 
