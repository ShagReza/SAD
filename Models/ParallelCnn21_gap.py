...
def create_model(input_shape1,input_shape2, config, is_training=True):
    ...
    for i in range(len(config.P2_cnns)):
        x2 = Convolution1D(config.P2_cnns[i], config.P2_kernel[i], padding=config.P2_pad, kernel_initializer = 'he_normal', activation="relu")(x2)
        x2=BatchNormalization()(x2)
        x2=MaxPooling1D(config.P2_pool_size, config.P2_strides)(x2)
